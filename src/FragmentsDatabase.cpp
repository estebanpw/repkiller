...
bool FragmentsDatabase::readFragment(FragFile * frag, FILE *f) {
    ...
    if (__BYTE_ORDER__ == __ORDER_BIG_ENDIAN__) {
      //big endian
      if (fread(&frag->diag, sizeof(int64_t), 1, f) != 1)     return false;
      if (fread(&frag->xStart, sizeof(uint64_t), 1, f) != 1)  return false;
      if (fread(&frag->yStart, sizeof(uint64_t), 1, f) != 1)  return false;
      if (fread(&frag->xEnd, sizeof(uint64_t), 1, f) != 1)    return false;
      if (fread(&frag->yEnd, sizeof(uint64_t), 1, f) != 1)    return false;
      if (fread(&frag->length, sizeof(uint64_t), 1, f) != 1)  return false;
      if (fread(&frag->ident, sizeof(uint64_t), 1, f) != 1)   return false;
      if (fread(&frag->score, sizeof(uint64_t), 1, f) != 1)   return false;
      if (fread(&frag->similarity, sizeof(float), 1, f) != 1) return false;
      if (fread(&frag->seqX, sizeof(uint64_t), 1, f) != 1)    return false;
      if (fread(&frag->seqY, sizeof(uint64_t), 1, f) != 1)    return false;
      frag->seqY = 1;
      if (fread(&frag->block, sizeof(int64_t), 1, f) != 1) return false;
      frag->strand = fgetc(f);
      if (fread(&frag->evalue, sizeof(long double), 1, f) != 1) return false;
      return true;
  } else {
      char tmpArray[sizeof(long double)];
      //little endian
      if (fread(tmpArray, sizeof(int64_t), 1, f) != 1)      return false;
      endianessConversion(tmpArray, (char *) (&frag->diag), sizeof(int64_t));
      if (fread(tmpArray, sizeof(uint64_t), 1, f) != 1)     return false;
      ...
      if (fread(tmpArray, sizeof(float), 1, f) != 1) return false;
      ...
      frag->strand = fgetc(f);
      if (fread(tmpArray, sizeof(long double), 1, f) != 1) return false;
